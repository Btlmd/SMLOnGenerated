...
from datasets import selfgen_labels
...
id_to_trainid = selfgen_labels.rbg2num
trainid_to_oodid = selfgen_labels.num2ood
num_classes = 24
ignore_label = 255
...
img_postfix = '.png'
mask_postfix = '.png'
mean = [0.50446028, 0.48020745, 0.45010477]
std = [0.26168964, 0.26381357, 0.26656617]
...
def add_items(items, aug_items, scenes, mode, maxSkip):
    """
    Add More items ot the list from the augmented dataset
    """
    for c in scenes:
        c_items = [name.split(img_postfix)[0] for name in
                   os.listdir(os.path.join(c, "rgb_v"))]
        for it in c_items:
            item = (os.path.join(c, "rgb_v", it + img_postfix),
                    os.path.join(c, "mask_v", it + mask_postfix))
            ########################################################
            ###### dataset augmentation ############################
            ########################################################
            if mode == "train" and maxSkip > 0:
                prev_seq_id = str(int(it) - maxSkip)
                next_seq_id = str(int(it) + maxSkip)

                prev_item = (os.path.join(c, "rgb_v", prev_seq_id + img_postfix),
                    os.path.join(c, "mask_v", prev_seq_id + mask_postfix))
                if os.path.isfile(prev_item[0]) and os.path.isfile(prev_item[1]):
                    aug_items.append(prev_item)
                next_item = (os.path.join(c, "rgb_v", next_seq_id + img_postfix),
                    os.path.join(c, "mask_v", next_seq_id + mask_postfix))
                if os.path.isfile(next_item[0]) and os.path.isfile(next_item[1]):
                    aug_items.append(next_item)
                # import IPython
                # IPython.embed(header="after perv_item")
            items.append(item)
    # import IPython
    # IPython.embed(header="after add_item")
...
def make_dataset(quality, mode, maxSkip=0, fine_coarse_mult=6, cv_split=0):
    ...
    if quality == 'coarse':
        ...
        # img_path = "./selfgen/normal/"
        # # mask_path = os.path.join(root, 'gtFine_trainvaltest', 'gtFine')
        # mask_postfix = '.png'
        # cv_splits = make_cv_splits(img_dir_name)
        data_dir = "/DATA2/gaoha/liumd/sml/sml/selfgen/selfgen/normal"
        scenes = os.listdir(data_dir)
        scenes = sorted(scenes)
        scenes = [os.path.join(data_dir, s, "1") for s in scenes]
        val_size = 1
        cv_split = {
            "train": scenes[val_size:],
            "val": scenes[:val_size]
        }
        if mode == 'trainval':
            modes = ['train', 'val']
        ...
        for mode in modes:
            if mode == 'test':
                ...
                add_items(items, aug_items, cv_split[mode], mode, maxSkip)
    else:
        raise 'unknown quality {}'.format(quality)
    # logging.info('Cityscapes-{}: {} images'.format(mode, len(items)))
    logging.info('SelfGen-{}: {} images'.format(mode, len(items) + len(aug_items)))
...
class SelfGen(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        mask_copy = np.asarray(original_mask).astype(np.uint32)
        ...
        assert seg_mask_copy.min() > 0, "original image should be fully labelled @ " + self.imgs[index]
        assert seg_mask_copy.max() < 23, "no anormaly should be in original image @ " + self.imgs[index]

        ood_mask_copy = np.zeros_like(mask, dtype=int)
        for k, v in trainid_to_oodid.items():
            ood_mask_copy[seg_mask_copy == k] = v
        ...
        # embed(header="after mask conversion sg")
...
class SelfGenUniform(data.Dataset):
    """
    ...
    def __init__(self, quality, mode, maxSkip=0, joint_transform_list=None, sliding_crop=None,
        ...
        self.imgs, self.aug_imgs = make_dataset(quality, mode, self.maxSkip)
        assert len(self.imgs), 'Found 0 images, please check the data set'

        

        # Centroids for fine data
        json_fn = 'selfgen_{}_cv{}_tile{}.json'.format(
        ...
        # import IPython
        # IPython.embed(header="after centroids")

        # Centroids for augmented data
        if self.maxSkip > 0:
            json_fn = 'selfgen_{}_cv{}_tile{}_skip{}.json'.format(
    ...
    def __getitem__(self, index):
        ...
        img, original_mask = Image.open(img_path).convert('RGB'), Image.open(mask_path)
        img_name = os.path.splitext(os.path.basename(img_path))[0]
        ...
        # from IPython import embed
        # embed(header="after mask conversion sg uniform")
